...
export class EmpresaUserService {
  url:string=dominio_ws+'/empresa_user';
  tabla:string='Empresa-Usuario';
  ...
  cargarDatosID(pk_user:number,pk_empresa:number):Observable<any>{
    let url_ws=`${this.url}/${pk_user}/${pk_empresa}`;
    return this.http.get(url_ws)
    .pipe(map((resp:any) =>{
        let dato={};
        if(resp.status === 'error'){
