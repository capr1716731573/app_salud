...
export class TipoOrganosSistemasService {
  
  url:string=dominio_ws+'/tipo_organos_sistemas';
  tabla:string='Tipo Organos y Sistemas ';
  ...
  cargarDatosBusqueda(padre:number,palabra:string){
    let url_ws=`${this.url}/busqueda/${palabra}/${padre}`;
    return this.http.get(url_ws)
    .pipe(map((resp:any) =>{
        let dato={};
        if(resp.status === 'error'){
